...
// costante con qualcosa tipo "PointLab <no-reply@tuodominio.it>".
const FROM = process.env.RESEND_FROM || 'PointLab <onboarding@resend.dev>';
...
function wrapHtml(title, bodyHtml) {
  ...
  <div style="font-family:Arial,sans-serif;background:#101d16;padding:32px 16px;">
    <div style="max-width:480px;margin:0 auto;background:#1b2e22;border-radius:14px;padding:28px 24px;color:#eef2ea;">
      <div style="font-size:12px;letter-spacing:1px;text-transform:uppercase;color:#d7ff4e;margin-bottom:6px;">🎾 PointLab</div>
...
export async function sendSubscriptionConfirmedEmail({ toEmail, coachName, planLabel, quota, periodEndFormatted }) {
  ...
  const html = wrapHtml('Abbonamento attivato ✅', `
    <p>Ciao ${coachName || ''},</p>
    <p>Il tuo abbonamento <b>${planLabel}</b> (fino a ${quota} partite registrabili) è attivo da subito.</p>
...
export async function sendTrainingPublishedEmail({ toEmail, athleteName, coachName, shotLabel }) {
  ...
  `);
  return resend.emails.send({ from: FROM, to: toEmail, subject: 'Il tuo maestro ha pubblicato un nuovo allenamento', html });
}
