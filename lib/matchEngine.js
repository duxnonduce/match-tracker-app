...
export function rebuildMatchFromLog(format, log) {
  ...
  for (const entry of log) {
    ...
    newLog.push({
      path: path.slice(), player, ts: entry.ts, pointWinner, set: setNumber, game: gameNumber, msBefore,
      // Metadati facoltativi del punto: li portiamo avanti tali e quali se
      // c'erano già (es. modificando un episodio dalla Timeline non si
      // deve perdere la direzione o la zona dell'errore già registrate).
      direction: entry.direction || null,
      errorZone: entry.errorZone || null,
      server: entry.server || null,
      serveNumber: entry.serveNumber || null,
      wasBreakPoint: entry.wasBreakPoint || false,
      breakPointResult: entry.breakPointResult || null,
    });
  }
...
/* ============================================================
   DIGEST PER IL COMMENTO TECNICO AI
   Trasforma stats/log grezzi in un riassunto pulito e strutturato,
   pensato per essere mandato a un modello linguistico — meglio dare
   dati già digeriti che un dump grezzo: risposte più precise, meno
   token spesi, e nessun rischio che il modello "inventi" numeri.
============================================================ */

function formatSetScoreDigest(ms) {
  if (!ms || !ms.completedSets) return null;
  const parts = ms.completedSets.map(s => {
    if (s.isMatchTiebreak) return `${s.allievo}-${s.avversario} (match tiebreak)`;
    return `${s.allievo}-${s.avversario}`;
  });
  if (ms.currentSetGames && (ms.currentSetGames.allievo > 0 || ms.currentSetGames.avversario > 0) && !ms.matchOver) {
    parts.push(`${ms.currentSetGames.allievo}-${ms.currentSetGames.avversario} (in corso)`);
  }
  return parts.join(', ');
}

export function computeMatchDigest(record) {
  ...
  const shotBreakdown = [];
  GROUPED_SHOTS.forEach(g => {
    let winner = 0, ef = 0, enf = 0;
    SUBFASI.forEach(f => {
      const rs = stats[g.key][f.key];
      winner += rs.winner.allievo; ef += rs.errori_forzati.allievo; enf += rs.errori_non_forzati.allievo;
    });
    shotBreakdown.push({ label: g.label, winner, errori_forzati: ef, errori_non_forzati: enf, net: winner - ef - enf });
  });
  SIMPLE_SHOTS.forEach(sh => {
    const rs = stats[sh.key];
    shotBreakdown.push({
      label: sh.label, winner: rs.winner.allievo, errori_forzati: rs.errori_forzati.allievo,
      errori_non_forzati: rs.errori_non_forzati.allievo, net: rs.winner.allievo - rs.errori_forzati.allievo - rs.errori_non_forzati.allievo,
    });
  });

  const rispostaBreakdown = RISPOSTA_FASI.map(f => {
    const rs = stats.risposta[f.key];
    return { fase: f.label, winner: rs.winner.allievo, errori_forzati: rs.errori_forzati.allievo, errori_non_forzati: rs.errori_non_forzati.allievo };
  });

  const serveStats = computeServeStats(log);

  const zoneCounts = {};
  let zoneTotal = 0;
  const directionCounts = { lungolinea: 0, diagonale: 0 };
  log.forEach(e => {
    if (e.player !== 'allievo') return;
    if (e.errorZone) { zoneCounts[e.errorZone] = (zoneCounts[e.errorZone] || 0) + 1; zoneTotal++; }
    if (e.direction) directionCounts[e.direction] = (directionCounts[e.direction] || 0) + 1;
  });
  ...
  return {
    ...
    setScore: formatSetScoreDigest(ms),
    totalPoints: log.length,
    shotBreakdown,
    rispostaBreakdown,
    servizio: { ace: stats.servizio.ace.allievo, doppiFalli: stats.servizio.doppio_fallo.allievo },
    serveStatsAvanzate: serveStats ? serveStats.allievo : null,
    zoneErrori: zoneTotal > 0 ? { conteggio: zoneCounts, totale: zoneTotal } : null,
    direzioni: (directionCounts.lungolinea + directionCounts.diagonale) > 0 ? directionCounts : null,
  };
}

/* ============================================================
   ANALISI AVANZATE — aggregazione su più partite e confronto diretto
   tra due partite. Usate dalla sezione "Statistiche" della dashboard.
============================================================ */

/** Somma ricorsivamente due oggetti statistiche con la stessa forma (foglie numeriche). */
function sumStatsObjects(a, b) {
  if (typeof a === 'number') return a + (typeof b === 'number' ? b : 0);
  if (a && typeof a === 'object') {
    const result = {};
    for (const k of Object.keys(a)) result[k] = sumStatsObjects(a[k], b ? b[k] : undefined);
    return result;
  }
  return a;
}

function matchOutcome(ms) {
  if (!ms || !ms.matchOver || !ms.winner) return null;
  return ms.winner; // 'allievo' | 'avversario'
}

/**
 * Aggrega un elenco di partite (record completi: meta/stats/log/match/coach_rating)
 * in un unico riepilogo — stessa "forma" del digest di una singola partita,
 * cosi le stesse funzioni di visualizzazione si possono riusare.
 */
export function aggregateMatches(records) {
  if (!records || records.length === 0) return null;

  const combinedStats = records.reduce((acc, r) => sumStatsObjects(acc, r.stats), initStats());
  const combinedLog = [];
  records.forEach(r => combinedLog.push(...(r.log || [])));

  const fakeRecord = { stats: combinedStats, log: combinedLog, meta: { allievo: 'Aggregato', avversario: '', formatLabel: null } };
  const digest = computeMatchDigest(fakeRecord);

  const ratings = records.filter(r => r.coach_rating != null).map(r => ({ date: r.meta?.data, matchId: r.id, rating: r.coach_rating }));
  const avgRating = ratings.length ? ratings.reduce((s, r) => s + r.rating, 0) / ratings.length : null;

  let wins = 0, losses = 0;
  records.forEach(r => {
    const outcome = matchOutcome(r.match);
    if (outcome === 'allievo') wins++;
    else if (outcome === 'avversario') losses++;
  });
  ...
  return {
    ...
    wins, losses,
    avgRating,
    ratingTrend: ratings.sort((a, b) => new Date(a.date) - new Date(b.date)),
    ...digest,
  };
}

/**
 * Confronta due partite (o due aggregati, stessa forma da computeMatchDigest/
 * aggregateMatches) e ritorna, per ogni colpo e per il servizio, la differenza
 * numerica tra la seconda ("dopo") e la prima ("prima") — utile per capire
 * cosa è migliorato o peggiorato.
 */
export function compareDigests(before, after) {
  const shotComparison = before.shotBreakdown.map((rowBefore) => {
    const rowAfter = after.shotBreakdown.find(r => r.label === rowBefore.label) || { net: 0, winner: 0, errori_forzati: 0, errori_non_forzati: 0 };
    return {
      label: rowBefore.label,
      before: rowBefore.net,
      after: rowAfter.net,
      delta: rowAfter.net - rowBefore.net,
    };
  });

  const serveComparison = {
    ace: { before: before.servizio.ace, after: after.servizio.ace, delta: after.servizio.ace - before.servizio.ace },
    doppiFalli: { before: before.servizio.doppiFalli, after: after.servizio.doppiFalli, delta: after.servizio.doppiFalli - before.servizio.doppiFalli },
  };

  let advancedServeComparison = null;
  if (before.serveStatsAvanzate && after.serveStatsAvanzate) {
    const b = before.serveStatsAvanzate, a = after.serveStatsAvanzate;
    advancedServeComparison = {
      firstInPct: { before: b.firstInPct, after: a.firstInPct, delta: (a.firstInPct ?? 0) - (b.firstInPct ?? 0) },
      wonOnFirstPct: { before: b.wonOnFirstPct, after: a.wonOnFirstPct, delta: (a.wonOnFirstPct ?? 0) - (b.wonOnFirstPct ?? 0) },
      wonOnSecondPct: { before: b.wonOnSecondPct, after: a.wonOnSecondPct, delta: (a.wonOnSecondPct ?? 0) - (b.wonOnSecondPct ?? 0) },
    };
  }

  const improved = shotComparison.filter(r => r.delta > 0).sort((x, y) => y.delta - x.delta);
  const worsened = shotComparison.filter(r => r.delta < 0).sort((x, y) => x.delta - y.delta);

  return { shotComparison, serveComparison, advancedServeComparison, improved, worsened };
}
